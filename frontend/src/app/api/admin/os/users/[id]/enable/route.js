...
export async function POST(request, { params }) {
  ...
  try {
    ...
    const r = await sql`
      UPDATE os_users SET enabled = TRUE WHERE id = ${id} RETURNING username, enabled
    `;
    if (r.length === 0) {
    ...
    try {
      ...
      await sql`
        ...
        VALUES (${actor}, 'enable', ${r[0].username}, null)
      `;
    } catch { /* noop */ }

    return NextResponse.json({
      status: "success",
      message: `Cuenta '${r[0].username}' habilitada`,
      data: r[0],
    });
  } catch (err) {
    console.error("Error al habilitar cuenta:", err);
